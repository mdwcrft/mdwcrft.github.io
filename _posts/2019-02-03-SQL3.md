...
A more specific query would state the column names and use `WHERE` to apply a logical condition on a column using an *equality operator* (=,<,>,>=,<=,<> are the ISO standards). The precursor to comments is `--` in SQL.  
