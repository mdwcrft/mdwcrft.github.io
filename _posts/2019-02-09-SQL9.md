...
#### System functions
getdate(), max(), datediff(), dateadd(), Len(), count(), ltrim(), rtrim(), substring(), charindex(), coalesce(), cast(), isnull(), nullif(), concat()  
...
---  
# Views  
Similar to a function but NON-SCALAR, and no input is supplied. (SELECT FROM)  
